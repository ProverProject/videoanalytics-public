...
Vector Vector::EllipticalShift(double rx, double ry, double targetX, double targetY) const {
    if (rx == 0) {
        if (targetY > _y + ry) {
...
Vector Vector::EllipticalShiftMagnet(float rx, float ry, float targetX, float targetY) const {
    if (rx == 0) {
        if (targetY > _y + ry) {
            return Vector(_x, _y + ry);
        } else if (targetY < _y - ry) {
...
Vector Vector::ShiftEllipseToTouchLineMagnet(float a, float b) const {
    if (a == 0) {
...
Vector Vector::RectShiftMagnet(float rx, float ry, float targetX, float targetY) const {
    float x = (float) _x;
    float x1 = x + rx;
    if (targetX < x1) {
        x1 = x - rx;
...
Vector Vector::ShiftRectToTouchLineMagnet(float rx, float ry) const {
    double x = _x - rx;
    double y = _y + ry;
    if (x < y) {
        return Vector(0, 0);
